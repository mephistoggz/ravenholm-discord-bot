...
client.login(process.env.token); // Bot tokeni.
...
mongoose.connect('KENDİ MONGODB LİNKİNİZ', {useNewUrlParser: true, useUnifiedTopology: true});// Mongo connect linki
...
return message.channel.send(`${message.author}, **${süre.seconds}** saniye beklemen gerekiyor.`).then(msg => {
  ...
  return
    ...
    message.react("EMOJIID").catch(err => message.channel.send({content: "Tepki ekleme iznim yok. Bulunduğun sunucunun yetkilisine ileterek bunu düzeltmesini iste.", ephemeral: true}))
...
//İTİRAF SİSTEMİ
client.on("messageCreate", async message => {
  const reklam = ["discord.gg", "dc.gg", "discord.com", ".com", ".gg", ".me", ".net", "www.", ".org", ".gov", ".tr", ".co", "https", "d i s c o r d", "bit.ly", "link.tl", "goo.gl", "polr.me", "tinyurl", "yourls", "ow.ly", "bl.ink", "adf.ly",];
  
  if(message.author.bot === true) return;
  if(message.channel.id !== "1115346115403980891") return;
  if(reklam.some(word => message.content.includes(word))) return;
...
//KÜFÜR SPAM CAPS REKLAM KORUMASI
//KÜFÜR
client.on("messageCreate", async message => {
  ...
  if(message.member.permissions.has(PermissionFlagsBits.Administrator) ||
     message.member.roles.cache.has("1089536569746141334") ||
     message.member.roles.cache.has("1089537013889372200") ||
     message.author.bot === true) 
     return;
  
const küfürler = db.fetch(`küfürler.${message.guild.id}`) || ["amk", "siktir g"]

if(küfürler.some(word => message.content.includes(word))) {
  
  message.channel.send({embeds: [embed.setDescription(`${message.author}, küfür edemezsin.`)]}).then(msg => { setTimeout(() => msg.delete(), 5000)})
...
if(message.author.bot === true) return;
  ...
  if(message.member.permissions.has(PermissionFlagsBits.Administrator) ||
    ...
    if(parseInt(msgcount) === 5) {
      ...
      setTimeout(() => {
        embed.setDescription(`${message.author}, spam yapamazsın.`)
...
if (Math.round(yuzde) > 50) {
  ...
  message.channel.send({embeds: [embed.setDescription(`${message.author}, caps kullanamazsın.`)]}).then(msg => { setTimeout(() => msg.delete(), 5000)})
...
if(reklam.some(word => message.content.includes(word))) {
  ...
  message.channel.send({embeds: [embed.setDescription(`${message.author}, reklam yapamazsın.`)]}).then(msg => { setTimeout(() => msg.delete(), 5000)})
...
if(reklam.some(word => newMessage.content.includes(word))) {
  ...
  newMessage.channel.send({embeds: [embed.setDescription(`${newMessage.author}, reklam yapamazsın.`)]}).then(msg => { setTimeout(() => msg.delete(), 5000)})
