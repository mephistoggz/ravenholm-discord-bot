...
exports.run = async (client, message, args) => {
  ...
  if (!message.member.permissions.has(PermissionFlagsBits.Administrator) ||
      ...
      !message.member.roles.cache.has("1114846200932352000")
  ...
  if(uyarı+1 == 1){
    ...
    embed.setDescription(`${user}, ilk uyarısını aldığı için Timeout uygulandı. Devam edersen sunucudan yasaklanabilirsin.`)
  ...
  if(uyarı+1 == 2){
    ...
    embed.setDescription(`${user}, ikinci uyarısını aldığı için Timeout uygulandı. Devam edersen sunucudan yasaklanabilirsin.`)
    return message.channel.send({embeds: [embed]})
  }
  ...
  if(uyarı+1 == 3){
    ...
    embed.setDescription(`${user}, üçüncü uyarısını aldığı için Jail'e gönderildi. Devam edersen sunucudan yasaklanabilirsin.`)
    return message.channel.send({embeds: [embed]})
  }
  ...
  if(uyarı+1 == 4){
    ...
    embed.setDescription(`${user}, son uyarıya geldiği için sunucudan yasaklandı.`)
